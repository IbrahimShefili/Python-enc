...
from python_sezar import CaesarCipherGUI
import credentials as cr

class login_page:
    def __init__(self, root):
        self.window = root
        self.window.title("Log In OurWindow")
        # Set the window size
        ...
        label1 = Label(self.frame1, text= "Our", font=("times new roman", 40, "bold"), bg="yellow", fg="red").place(x=100,y=300)
        label2 = Label(self.frame1, text= "Window", font=("times new roman", 40, "bold"), bg="yellow", fg="RoyalBlue1").place(x=162,y=300)
        label3 = Label(self.frame1, text= "It's all about cyrptography", font=("times new roman", 13, "bold"), bg="yellow", fg="brown4").place(x=100,y=360)
        ...
        #================Buttons===================
        self.login_button = Button(self.frame3,text="Log In",command=self.login_func,font=("times new roman",14, "bold"),bd=0,cursor="hand2",bg="blue",fg="white").place(x=50,y=200,width=300)

        self.forgotten_pass = Button(self.frame3,text="Forgotten password?",command=self.forgot_func,font=("times new roman",9, "bold"),bd=0,cursor="hand2",bg="white",fg="blue").place(x=125,y=250,width=150)

        self.create_button = Button(self.frame3,text="Create New Account",command=self.redirect_window,font=("times new roman",14, "bold"),bd=0,cursor="hand2",bg="green2",fg="white").place(x=80,y=290,width=250)

        self.create_button = Button(self.frame3,text="1.Go To Sezar Chiper",command=self.redirect_window2,font=("times new roman",9, "bold"),bd=0,cursor="hand2",bg="white",fg="black").place(x=80,y=340,width=150)
    ...
    def login_func(self):
        if self.email_entry.get()=="" or self.password_entry.get()=="":
            ...
            try:
                connection=pymysql.connect(host=cr.host, user="user1", password="Bhole@89", database=cr.database)
