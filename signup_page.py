...
class SignUp:
    def __init__(self, root):
        ...
        self.bg_img = ImageTk.PhotoImage(file="Images/pex.jpg")
        background = Label(self.window,image=self.bg_img).place(x=0,y=0,relwidth=1,relheight=1)


        frame = Frame(self.window, bg="white")
        frame.place(x=270,y=70,width=670,height=670)

        title1 = Label(frame, text="Sign Up", font=("times new roman",25,"bold"),bg="white").place(x=20, y=10)
        title2 = Label(frame, text="Bizə qatılın", font=("times new roman",13),bg="white", fg="gray").place(x=20, y=50)

        f_name = Label(frame, text="Ad", font=("helvetica",15,"bold"),bg="white").place(x=20, y=100)
        l_name = Label(frame, text="Soyad", font=("helvetica",15,"bold"),bg="white").place(x=240, y=100)
        ...
        sec_question = Label(frame, text="Təhlükəsizlik sualı", font=("helvetica",15,"bold"),bg="white").place(x=20, y=260)
        answer = Label(frame, text="Cavab", font=("helvetica",15,"bold"),bg="white").place(x=240, y=260)

        self.questions = ttk.Combobox(frame,font=("helvetica",12),state='readonly',justify=CENTER)
        self.questions['values'] = ("Seçin","Ev heyvanınızın adı nədir?","Ləqəbiniz","Ad gününüz nə zamandır", "Sevimli filmizniz")
        ...
        password =  Label(frame, text="Yeni şifrə", font=("helvetica",15,"bold"),bg="white").place(x=20, y=340)

        self.password_txt = Entry(frame,font=("arial"))
        self.password_txt.place(x=20, y=370, width=420)

        self.terms = IntVar()
        terms_and_con = Checkbutton(frame,text="Şərtlərlə razıyam",variable=self.terms,onvalue=1,offvalue=0,bg="white",font=("times new roman",12)).place(x=20,y=420)
